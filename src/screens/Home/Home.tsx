...
const Home: React.FC<HomeProps> = ({ navigation, route }) => {
    ...
    const [showNoticeLogOut, setShowNoticeLogOut] = useState(false);
    ...
    useEffect(() => {
        if (route.params.user !== null && route.params.user._id !== '') {
            setUser(route.params.user);
            if (route.params.prevAddress === 'Login' || route.params.prevAddress === 'Register') {
                setShowNotice(true);
            }
        }
        if (route.params.prevAddress === 'Profile' && route.params.user === null) {
            setUser({
                _id: '',
                email: '',
                username: '',
    ...
    if (showNoticeLogOut) {
        setTimeout(() => setShowNoticeLogOut(false), 2000);
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <Modal transparent visible={showNoticeLogOut} animationType="fade">
                <Notice type={'success'} text={'Bạn đã đăng xuất thành công!'} />
            </Modal>
